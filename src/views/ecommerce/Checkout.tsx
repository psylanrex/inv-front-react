...
export default function Checkout() {
  ...
  const data_checkout = {
    ...
    payment: [
      {
        name: "Credit/Debit",
        img: "/img/payments/creditcard.png",
        default: true,
      },
  ...
  const radioAddress = (e: any) => {
    setAddress(e.target.value);
  };
  const radioCurrier = (e: any) => {
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap flex-row mb-6">
                  {data_address.map((address, index) => (
                    <label
                      key={index}
                      className="p-6 flex-shrink bg-gray-50 dark:bg-gray-900 bg-opacity-70 max-w-full w-full md:w-1/2 cursor-pointer"
                    >
                      <input
                        type="radio"
                        onChange={radioAddress}
                      ...
                      <p className="text-gray-500">
                        ...
                        {address.address}
                        <br />
                        <abbr title={checkout_text.phone}>
                          {checkout_text.phone}:
                        </abbr>{" "}
                        {address.phone}
                      </p>
                ...
                <div className="flex flex-wrap flex-row mb-6 bg-gray-50 dark:bg-gray-900 bg-opacity-70 text-gray-700">
                  {data_shipping.map((shipping, index) => (
                    <label
                      key={index}
                      className="p-6 flex-shrink max-w-full w-full md:w-1/3 cursor-pointer"
                    >
                      <input
                        type="radio"
                        onChange={radioCurrier}
            ...
            <table className="table-auto w-full ltr:text-left rtl:text-right text-gray-600">
              ...
              <tfoot className="bg-gray-100 dark:bg-gray-900 bg-opacity-60 dark:bg-opacity-20 text-sm">
                <tr className="border-b dark:border-gray-700">
                  <td className="p-4 font-normal text-center ltr:text-right rtl:text-left">
                    <b>{checkout_text.sub}</b>
                  </td>
