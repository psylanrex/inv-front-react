...
export default function ProjectTable(props) {
  ...
  return (
    <table className="table-bordered w-full ltr:text-left rtl:text-right text-gray-500 dark:text-gray-400">
      <thead>
        <tr>
          <th>{project_detail.table_title}</th>
          <th>{project_detail.status}</th>
          <th className="hidden lg:table-cell">{project_detail.assigned}</th>
          <th className="hidden lg:table-cell">{project_detail.time}</th>
          <th>{project_detail.action}</th>
        </tr>
      </thead>
        ...
        {data_table.map((table, index) => {
          ...
          return (
            <tr key={index}>
              ...
              <td>
                <div
                  className={`text-sm text-center px-2 py-1 font-semibold leading-tight rounded-full ${status_color}`}
                >
                  {table.status}
                </div>
              </td>
