  ...
  return (
    <Card className="!p-0">
        ...
        <div className="ltr:float-right rtl:float-left">
          {data_progress > 1 ? (
            <Tooltip title="Since last month">
              <p className="text-green-500">+{data_progress.toFixed(0)}%</p>
            </Tooltip>
          ) : (
            <Tooltip title="Since last month">
              <p className="text-red-500">{data_progress.toFixed(0)}%</p>
            </Tooltip>
          )}
        </div>
      </div>
      <div className="flex flex-row justify-between px-6 py-4">
        <div
          className={`flex items-center justify-center w-14 h-14 rounded-full dark:bg-opacity-40 relative text-center ${addcolor}`}
        >
          {data.icon}
        </div>
        <h2 className="self-center text-3xl">
          <ShortFormat data={data.new} />
        </h2>
      </div>
      <div className="px-6 pb-6">
        <a className="hover:text-indigo-500 text-sm" href={data.url}>
          {data.text}
        </a>
      </div>
    </Card>
