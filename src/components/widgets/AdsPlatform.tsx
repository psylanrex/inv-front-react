    ...
    <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg h-full">
      <div className="flex flex-row justify-between items-center pb-6">
        ...
        <button className="cursor-pointer">
          <div
            title="Show"
            className="px-2 rounded-full border border-gray-200 dark:border-gray-800 text-green-600 bg-gray-50 dark:bg-gray-900 hover:bg-green-200 hidden"
          >
            +
          </div>
        </button>
      ...
      <div className="relative">
        <div className="flex flex-wrap flex-row">
          {data_ads.map((item, index) => (
            <div
              key={index}
              className="flex-shrink max-w-full px-0.5 w-1/2 sm:w-1/3 xl:w-1/4 mb-1"
            >
              <div className="p-2 w-full bg-gray-50 dark:bg-gray-900">
                <h4 className="text-sm font-bold mb-1">
                  {item.type === "currency" ? (
                    <Currency data={item.value} />
                  ) : item.type === "percent" ? (
                    item.value + "%"
                  ) : (
                    item.value
