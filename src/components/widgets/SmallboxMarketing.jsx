...
export default function SmallboxMarketing(props) {
  ...
  const color_percent = percent > 80 ? "text-green-500" : percent > 60 ? "text-indigo-500" : percent > 40 ? "text-yellow-500" : percent > 20 ? "text-gray-500" : "text-red-500";
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg h-full p-6 relative overflow-hidden">
      ...
      <Progress percent={percent.toFixed(0)} type={props.data.type} data={data_count} model="circle" color={props.color} />
      <p className="text-sm text-center font-semibold"><span className={`font-bold ${color_percent}`}>{percent.toFixed(0)}%</span> of {dataType === 'currency' ? <Currency data={data_target} /> : data_target }</p>
