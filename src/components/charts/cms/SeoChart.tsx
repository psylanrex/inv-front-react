...
export default function SeoChart(props) {
  ...
  const data = {
    ...
    datasets: [
      {
        data: [85, 15],
        backgroundColor: [text_green_500, hexToRGBA(text_gray_500, 0.2)],
        borderColor: [text_green_500, hexToRGBA(text_gray_500, 0.2)],
        borderWidth: 1,
      },
    ],
  };
  ...
  return (
    <Card className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
      ...
      <div className="relative my-min-18">
        ...
        <div className="absolute ltr:left-1/2 rtl:right-1/2 top-1/2 transform ltr:-translate-x-1/2 rtl:translate-x-1/2 translate-y-full">
          {numbers}
        </div>
      </div>
        ...
        <div className="py-3">
          {datainfos.map((datainfo, index) => (
            <div key={index} className="inline-block ltr:mr-2 rtl:ml-2">
              <span
                className={`inline-block w-3 h-3 ltr:mr-1 rtl:ml-1 rounded-full ${datainfo.color}`}
              ></span>{" "}
              {datainfo.type}
            </div>
          ))}
