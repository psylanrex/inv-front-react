...
export default function StorangeChart(props) {
    ...
    datasets: [
      {
        label: "Storage",
        data: [90000, 52000],
        backgroundColor: [text_primary_500, text_green_500],
        hoverOffset: 4,
      },
    ],
  };
  ...
  const percent =
    (hosting_space.used / (hosting_space.used + hosting_space.free)) * 100;
  ...
  return (
    <Card className="h-full">
      ...
      <div className="text-center text-sm font-semibold text-gray-500 mt-6">
        <p className="mb-2">
          <span className="inline-block w-4 h-4 bg-indigo-500 mr-2"></span>
          {hosting_text.used}: {hosting_space.used}{" "}
          <span className="ltr:ml-4 rtl:mr-4 inline-block w-4 h-4 bg-green-500 mr-2"></span>
          {hosting_text.free}: {hosting_space.free}
        </p>
      </div>
    </Card>
  );
