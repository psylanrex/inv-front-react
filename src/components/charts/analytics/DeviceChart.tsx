...
export default function DeviceChart(props) {
    ...
    datasets: [
      {
        ...
        data: [925, 30, 552],
        backgroundColor: [text_primary_500, text_secondary_500, text_green_500],
        hoverOffset: 4,
      },
    ],
  };
  ...
  const laptop_progress =
    ((data_laptop.new - data_laptop.old) / data_laptop.new) * 100;
  ...
  const mobile_progress =
    ((data_mobile.new - data_mobile.old) / data_mobile.new) * 100;
    ...
    <Card className="h-full">
      ...
      <div className="flex flex-row justify-between my-6">
        <div className="block text-center px-3">
          ...
          <h5 className="text-xl font-bold block py-2">
            {isActive === 1
              ? `${laptop_percent.toFixed(0)}%`
              : `${laptop_percentOld.toFixed(0)}%`}{" "}
          </h5>
          <div className="block items-center">
            {laptop_progress > 1 ? (
              <p className="flex items-center text-sm text-green-500">
                <ArrowUpShort className="inline-block w-4 h-4" />{" "}
                {laptop_progress.toFixed(1)}%
              </p>
            ) : (
        ...
        <div className="block text-center px-3">
          ...
          <h5 className="text-xl font-bold block py-2">
            {isActive === 1
              ? `${tabs_percent.toFixed(0)}%`
              : `${tabs_percentOld.toFixed(0)}%`}{" "}
          </h5>
          <div className="block items-center">
            {tabs_progress > 1 ? (
              <p className="flex items-center text-sm text-green-500">
                <ArrowUpShort className="inline-block w-4 h-4" />{" "}
                {tabs_progress.toFixed(1)}%
              </p>
            ) : (
              <p className="flex items-center text-sm text-pink-500">
                <ArrowDownShort className="inline-block w-4 h-4" />{" "}
        ...
        <div className="block text-center px-3">
          ...
          <h5 className="text-xl font-bold block py-2">
            {isActive === 1
              ? `${mobile_percent.toFixed(0)}%`
              : `${mobile_percentOld.toFixed(0)}%`}{" "}
          </h5>
          <div className="block items-center">
            {mobile_progress > 1 ? (
              <p className="flex items-center text-sm text-green-500">
                <ArrowUpShort className="inline-block w-4 h-4" />{" "}
                {mobile_progress.toFixed(1)}%
              </p>
            ) : (
              <p className="flex items-center text-sm text-pink-500">
                <ArrowDownShort className="inline-block w-4 h-4" />{" "}
