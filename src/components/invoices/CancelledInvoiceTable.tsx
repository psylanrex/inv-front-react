...
import { InvoiceData, getCancelledInvoices } from "@/api/invoice";
...
type DataCancelledInvoiceTable = {
  loading: boolean;
  keyword: string;
...
const CancelledInvoiceTable = () => {
  const navigate = useNavigate();
  const [state, setState] = useSetState<DataCancelledInvoiceTable>({
    loading: false,
    keyword: "",
    invoices: [],
    currentPage: 1,
  ...
  const fetchCancelledInvoice = async () => {
    setState({ loading: true });
    const [err, data] = await to(getCancelledInvoices());
    if (err) return setState({ loading: false });
    setState({ invoices: data, loading: false });
  };

  useAsyncEffect(async () => {
    await fetchCancelledInvoice();
  }, []);
            ...
            {currentData.map((invoice, id) => {
              ...
              return (
                <tr key={id} className={classRow}>
                  ...
                  <td>
                    <div
                      className={`text-sm px-2 py-1 font-semibold leading-tight text-center rounded-full text-red-700 bg-red-100`}
                    >
...
export default CancelledInvoiceTable;
